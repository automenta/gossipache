...
public class GossipCore implements GossipCoreConstants {
    ...
    private final ConcurrentHashMap<String, Consumer<Base>> requests;
    ...
    void send(Base message, URI uri, Consumer<Base> onResponse) {
        ...
        if (message instanceof Trackable) {
            t = (Trackable) message;
            if (onResponse!=null)
                requests.put(t.getUuid() + '/' + t.getUriFrom(), onResponse);
        }
...
//        }
    ...
    public void handleResponse(String k, Base v) {
        Consumer<Base> onResponse = requests.remove(k);
        if (onResponse == null)
            return;
